...
int startWith(const char *arr, const char *substring)
{
    int result = 0;
    ...
    do  {
    	...
    	if (fgets( line, 100, stdin ) == NULL) {
    		...
    		if (find == NULL)  {
				...
				char* pPosition = strchr(allowed_chars, lowercase_name[0]);
				if (pPosition == NULL)  {
					allowed_chars[allowed_chars_count++] = lowercase_name[0];
				}
    		} else {
    			if(startWith(lowercase_name, find) == 1) {
    				find_address_count++;
        			if (find_address_count == 1) {
            			strcpy(find_address,lowercase_name);
        			}
    				char* pPosition = strchr(allowed_chars, lowercase_name[strlen(find)]);
    				if (pPosition == NULL)  {
    					allowed_chars[allowed_chars_count++] = lowercase_name[strlen(find)];
    				}
    			}
    		}
    	}
